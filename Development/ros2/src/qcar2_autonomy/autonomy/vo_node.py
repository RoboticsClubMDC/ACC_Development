"""VO redundancy monitor built on depth-based visual odometry.
   LC CHANGED AFTER GABRIEL TO PUBLISH NEW DIAGNOSTICS AND SUPPORT PIT ALIGNED DEPTH INPUT. THIS IS JUST A TEST ONE, NOTICE IT.
...
class VONodeDepth(Node):
    ...
    def __init__(self):
        ...
        # LC CHANGE — depth-quality feature filter before RANSAC
        self.declare_parameter('use_depth_quality_filter', True)
        self.declare_parameter('depth_patch_radius', 2)
        self.declare_parameter('min_depth_valid_ratio', 0.45)
        self.declare_parameter('min_depth_quality', 0.20)
        ...
        use_depth_quality_filter = bool(
            self.get_parameter('use_depth_quality_filter').value)
        ...
        min_depth_quality = float(
            self.get_parameter('min_depth_quality').value)
        ...
        self.vo = VisualOdometryDepth(
            ...
            use_depth_quality_filter=use_depth_quality_filter,
        ...
        self.last_rgb_matches = 0# LC HERE TOO
        self.last_depth_valid = 0#LC2
        self.last_depth_quality_mean = 0.0#LC3
        ...
        #LC CHANGE — new diagnostics for VO feature matches and depth quality
        self.pub_rgb_matches = self.create_publisher(
            Int32, '/vo/rgb_matches', 5)
        self.pub_depth_valid = self.create_publisher(
            Int32, '/vo/depth_valid_matches', 5)
        self.pub_depth_quality = self.create_publisher(
            Float64, '/vo/depth_quality_mean', 5)
        ...
        self.get_logger().info(
            ...
            f"║  depth_quality_filter="
            f"{'ON' if use_depth_quality_filter else 'OFF'} "
            f"patch={depth_patch_radius} "
            f"qmin={min_depth_quality:.2f}\n"
            f"╚══════════════════════════════════════════════╝")
        ...
        if max_vo_feature_depth_m > 0.0:
            ...
            #LC CHANGE HERE TOO
        if use_depth_quality_filter:
            self.get_logger().warn(
                f"Depth-quality feature filter ACTIVE "
                f"(patch_radius={depth_patch_radius}, "
                f"min_valid_ratio={min_depth_valid_ratio}, "
                f"min_depth_quality={min_depth_quality}).")
    # ── Callbacks ───────────────────────────────────────────────
    ...
    def _vo_tick(self):
        ...
        self.last_rgb_matches = int(result.get('rgb_match_count', 0))
        self.last_depth_valid = int(result.get('depth_valid_count', 0))
        self.last_depth_quality_mean = float(
            result.get('depth_quality_mean', 0.0))
    ...
    def _publish_all(self, rho, weight, state, healthy,
        ...
        msg.data = (
            ...
            f"rgb={self.last_rgb_matches} "
            f"dvalid={self.last_depth_valid} "
            f"dq={self.last_depth_quality_mean:.2f} "
        ...
        rgb = Int32()
        rgb.data = int(self.last_rgb_matches)
        self.pub_rgb_matches.publish(rgb)

        dv = Int32()
        dv.data = int(self.last_depth_valid)
        self.pub_depth_valid.publish(dv)

        self.pub_depth_quality.publish(_f(self.last_depth_quality_mean))
