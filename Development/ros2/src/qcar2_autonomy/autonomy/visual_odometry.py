"""Depth-based visual odometry for QCar2.
    LC CHANGED VISUAL ODOMETRY AFTER GABRIEL THIS IS JUST A TEST ONE, NOTICE IT.
...
class DepthProjector:
    ...
    def pixels_to_3d_body(self, pixels, aligned_depth):
        ...
        return points_body, valid, depths
#===================================================================================
# LC CHANGE #1 - DEPTH Gaussian-Based ORB QUALIFICATION and Median depth estimation
#===================================================================================
    def pixels_to_3d_body_quality(self, pixels, aligned_depth,
                                  patch_radius=2,
                                  min_valid_ratio=0.45,
                                  min_quality=0.20):
        """Convert pixels to body-frame 3D using robust per-feature depth.

        Instead of using one raw depth pixel, this uses a local depth patch
        around each ORB feature:
            - median depth = robust feature depth
            - MAD/std = local depth noise estimate
            - percentile span = depth-edge / discontinuity detector
            - quality = [0, 1] trust score for this feature's depth

        Returns:
            points_body: Nx3 body-frame points
            valid: N bool mask
            depths: N median camera depths
            sigmas: N local robust depth sigma estimate
            qualities: N depth quality score [0, 1]
        """
        N = pixels.shape[0]
        points_body = np.zeros((N, 3), dtype=np.float64)
        depths = np.zeros(N, dtype=np.float64)
        ...
        patch_size = (2 * r + 1) ** 2
        min_valid_count = max(3, int(np.ceil(min_valid_ratio * patch_size)))

        # Convert absolute meter thresholds into active depth units.
        # physical_m_per_unit = 1.0 for physical, 0.1 for virtual QLabs.
        unit = max(float(self.physical_m_per_unit), 1e-9)
        sigma_floor_units = 0.015 / unit   # 1.5 cm equivalent
        edge_floor_units = 0.040 / unit    # 4 cm equivalent
        ...
        for i in range(N):
            ...
            patch = aligned_depth[v-r:v+r+1, u-r:u+r+1]
            ...
            mad = float(np.median(np.abs(valid - z_med)))
            sigma_z = 1.4826 * mad
            ...
            # Dynamic thresholds: allow slightly more noise as range grows.
            sigma_limit = max(sigma_floor_units, 0.035 * z_med)
            edge_limit = max(edge_floor_units, 0.080 * z_med)

            # Reject unstable patches and depth discontinuities.
            if sigma_z > sigma_limit:
                continue

            if span > edge_limit:
                continue

            q_valid = float(valid.size) / float(patch_size)
            q_noise = float(np.exp(- (sigma_z / max(sigma_limit, 1e-9)) ** 2))
            q_edge = float(np.exp(- (span / max(edge_limit, 1e-9)) ** 2))

            # Range penalty in real meters, not QLabs units.
            z_phys = z_med * unit
            q_range = 1.0 / (1.0 + (z_phys / 2.0) ** 2)

            q_depth = float(q_valid * q_noise * q_edge * q_range)

            if q_depth < min_quality:
                continue
        ...
        if not np.any(valid_mask):
            return points_body, valid_mask, depths, sigmas, qualities

        pix_valid = pixels[valid_mask].astype(np.float64)
        d_valid = depths[valid_mask]

        p_hom = np.hstack([pix_valid, np.ones((pix_valid.shape[0], 1))])

        K_inv = self.K_rgb_inv if self.depth_on_color_grid else self.K_depth_inv
        rays = (K_inv @ p_hom.T).T
        P_cam = d_valid.reshape(-1, 1) * rays

        P_body = (self.R_cam2body @ P_cam.T).T + self.t_cam2body
        points_body[valid_mask] = P_body
...
class VisualOdometryDepth:
    ...
    def __init__(self, img_width=640, img_height=480,
                 ...
                 #=================
                 # LC CHANGE #2 - Constructor patching about DEPTH Qualification and median
                 #=================
                 use_depth_quality_filter=True,
                 depth_patch_radius=2,
                 min_depth_valid_ratio=0.45,
                 min_depth_quality=0.20):
        ...
        #==============================================================
        #LC - CHANGE #3 - FEATURE-LEVEL DEPTH CONFIDENCE BEFORE RANSAC
        #==============================================================
        # Step 2 — feature-level depth confidence before RANSAC.
        # This filters depth at each ORB match using a local patch, so
        # RANSAC receives cleaner 3D candidates instead of raw depth pixels.
        self.use_depth_quality_filter = bool(use_depth_quality_filter)
        self.depth_patch_radius = max(1, int(depth_patch_radius))

        self.min_depth_valid_ratio = float(min_depth_valid_ratio)
        self.min_depth_valid_ratio = float(
            np.clip(self.min_depth_valid_ratio, 0.0, 1.0)
        )

        self.min_depth_quality = float(min_depth_quality)
        self.min_depth_quality = float(
            np.clip(self.min_depth_quality, 0.0, 1.0)
        )

        # Debug counters for diagnosing where confidence is dying.
        self.rgb_match_count = 0
        self.depth_valid_count = 0
        self.depth_quality_mean = 0.0
    ...
    def update(self, color_image, depth_raw, timestamp):
        ...
        result = {
            ...
            'valid': False,
            #==============================
            #LC CHANGE #4 - New diagnostics
            #==============================
            'rgb_match_count': 0,
            'depth_valid_count': 0,
            'depth_quality_mean': 0.0
        }
        ...
        #==========================
        # LC CHANGE #5 - MATCHES RGB
        #==========================
        self.rgb_match_count = len(matches)
        result['rgb_match_count'] = self.rgb_match_count
        ...
        #=============================================================================
        # LC - CHANGE #6 - DEPTH-AWARE BLOCK BELOW FOR MATCHING PIXEL TO 3D BODY-FRAME
        #=============================================================================
        # --- Backproject matched pixels to 3D body-frame ---
        # New path: robust feature-level depth validation BEFORE RANSAC.
        if self.use_depth_quality_filter:
            prev_3d, prev_valid, prev_depths, prev_sigmas, prev_q = \
                self.projector.pixels_to_3d_body_quality(
                    prev_pts,
                    self._prev_depth,
                    patch_radius=self.depth_patch_radius,
                    min_valid_ratio=self.min_depth_valid_ratio,
                    min_quality=self.min_depth_quality
                )

            curr_3d, curr_valid, curr_depths, curr_sigmas, curr_q = \
                self.projector.pixels_to_3d_body_quality(
                    curr_pts,
                    aligned_depth,
                    patch_radius=self.depth_patch_radius,
            ...
            # Old path: single-pixel depth lookup.
            ...
            prev_sigmas = np.zeros_like(prev_depths)
            curr_sigmas = np.zeros_like(curr_depths)
            prev_q = np.ones_like(prev_depths)
            curr_q = np.ones_like(curr_depths)

        # Both endpoints must have reliable depth.
        both_valid = prev_valid & curr_valid

        # Existing hard max-depth cutoff.
        ...
        self.depth_valid_count = int(np.sum(both_valid))
        result['depth_valid_count'] = self.depth_valid_count

        if self.depth_valid_count > 0:
            pair_q_all = np.minimum(prev_q[both_valid], curr_q[both_valid])
            self.depth_quality_mean = float(np.mean(pair_q_all))
        else:
            self.depth_quality_mean = 0.0

        result['depth_quality_mean'] = self.depth_quality_mean

        if self.depth_valid_count < self.min_inliers:
            self._store(keypoints, descriptors, aligned_depth, timestamp)
            return result

        # Extract 2D body-frame coordinates.
        prev_2d = prev_3d[both_valid, :2]
        curr_2d = curr_3d[both_valid, :2]
        valid_prev_px = prev_pts[both_valid]

        # Pair quality: a match is only as good as its weaker depth endpoint.
        pair_quality = np.minimum(prev_q[both_valid], curr_q[both_valid])

        # Build per-correspondence weights.
        weights = None

        if self.use_depth_quality_filter:
            weights = pair_quality.copy()
        ...
        if self.depth_weight_power > 0.0:
            ...
            range_weight = pair_range ** (-self.depth_weight_power)

            if weights is None:
                weights = range_weight
            else:
                weights = weights * range_weight

        # Normalize weights for stable RANSAC/SVD.
        if weights is not None:
            weights = np.asarray(weights, dtype=np.float64)
            weights[~np.isfinite(weights)] = 0.0
            weights = np.maximum(weights, 0.0)

            wmax = np.max(weights) if weights.size > 0 else 0.0
            if wmax > 0.0:
        ...
        #=====================================================================================
        #LC - CHANGE #7 - INLIER SPREAD as a quality metric, Confidence checking before RANSAC
        #=====================================================================================
        self.inlier_count = int(np.sum(inlier_mask))

        # Confidence should be measured against depth-valid candidates,
        # not raw RGB matches. Otherwise good RGB + bad depth can report
        # fake-low confidence like 20%.
        if weights is not None and np.sum(weights) > 0.0:
            self.confidence = float(np.sum(weights[inlier_mask]) /
                                    np.sum(weights))
        else:
            denom = max(prev_2d.shape[0], 1)
            self.confidence = float(self.inlier_count / denom)
    ...
    def _ransac_motion(self, pts_prev, pts_curr, weights=None):
        ...
        Step 1: `weights` (M,) is the per-correspondence depth quality
        weight. It is now used in three places:
            1. weighted random sampling, so clean depth points are more
               likely to generate hypotheses
            2. weighted consensus scoring, so the winning RANSAC model is
               supported by high-quality depth points
            3. weighted SVD refit, so final motion is pulled more by stable
               depth features

        weights=None preserves the old unweighted estimator.
        """
        M = pts_prev.shape[0]
        ...
        # Depth-quality-aware sampling probability.
        sample_prob = None
        if weights is not None:
            ...
            if np.count_nonzero(w_all) >= s and np.sum(w_all) > 0.0:
                sample_prob = w_all / np.sum(w_all)

        for _ in range(self.ransac_iterations):
            idx = np.random.choice(M, s, replace=False, p=sample_prob)
            ...
            # Old score: number of inliers.
            # New score: sum of depth-quality weights of inliers.
            if weights is not None and np.sum(weights) > 0.0:
                score = float(np.sum(weights[inl]))
            else:
                score = float(c)
