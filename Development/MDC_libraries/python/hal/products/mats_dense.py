...
class SDCSRoadMap(RoadMap):
    ...
    def __init__(self, leftHandTraffic=False, useSmallMap=False):
        ...
        # ============================================================
        # DENSIFICATION BLOCK (Right-hand + Big Map only)
        # Adds your new nodes (in QLabs meters) and connects each to the
        # nearest existing node with straight edges (radius=0).
        # ============================================================
        if (not leftHandTraffic) and (not useSmallMap):

            # Your clicked coordinates (x,y) in QLabs meters
            new_nodes_xy = [
                [ 2.160,  3.501],
                [ 1.546,  4.422],
                [ 0.297,  1.086],
            ...
            # Add nodes (heading set to 0; edges are straight so it won’t break geometry)
            new_node_ids = []
            for x, y in new_nodes_xy:
                self.add_node([float(x), float(y), 0.0])
                new_node_ids.append(len(self.nodes) - 1)

            # Helper: closest node by Euclidean distance
            def closest_node_id(x, y, ignore_ids):
                best_i = 0
                best_d = float("inf")
            ...
            # Connect each new node to nearest original node with straight edges.
            # NOTE: this increases graph resolution without guessing radii.
            # If you want directional-only, remove one of the add_edge calls.
            original_ids = set(range(24))  # original right-hand big map nodes are 0..23
            for nid, (x, y) in zip(new_node_ids, new_nodes_xy):
                near = closest_node_id(x, y, ignore_ids=set(new_node_ids))

                # near -> new, new -> near
                self.add_edge(near, nid, 0)
                self.add_edge(nid, near, 0)