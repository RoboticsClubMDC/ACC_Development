...
Setup
Make sure that you have gone through the entire ACC Software Setup Instructions and the Running Nav 2 Guide before continuing with this guide.

Open QLabs to the Plane World.
Open a new terminal (CTRL + ALT + T) and run the Quanser Virtual Environment Container.

bash    cd /home/$USER/Documents/ACC_Development/docker/virtual_qcar2
    sudo docker run --rm -it --network host --name virtual-qcar2 quanser/virtual-qcar2 bash

Spawn the QCar in the competition environment by running the following commands in the Quanser Virtual Environment Container.

bash    cd /home/qcar2_scripts/python
    python3 Base_Scenarios_Python/Setup_Competition_Map.py

Open a new terminal and start the Development Container.

bash    cd /home/$USER/Documents/ACC_Development/isaac_ros_common
    ./scripts/run_dev.sh /home/$USER/Documents/ACC_Development/Development

Build and source the nodes in ROS2.

bash    colcon build && . install/setup.bash && export ROS_DOMAIN_ID=67

Launch the Cartographer SLAM and navigation stack. The VO system depends on this for the camera topics and the odometry TF that it compares against.

bash    ros2 launch qcar2_nodes qcar2_slam_and_nav_bringup_virtual_launch.py

Start the autonomous navigation. Open a new terminal, attach to the Development Container, and run the autonomy planner.

bash    cd /home/$USER/Documents/ACC_Development/isaac_ros_common
    ./scripts/run_dev.sh /home/$USER/Documents/ACC_Development/Development
bash    . install/setup.bash && export ROS_DOMAIN_ID=67
...
Open a new terminal and attach to the Development Container.

bash    cd /home/$USER/Documents/ACC_Development/isaac_ros_common
    ./scripts/run_dev.sh /home/$USER/Documents/ACC_Development/Development

Start the VO node with the calibrated QLabs parameters.
...
Open another terminal, attach to the Development Container, and echo the supervisor mode to see the system's decisions in real time.
...
You can also verify all topics are publishing by running:

bash    ros2 topic list | grep "^/vo/"
You should see:
    /vo/delta_trans
    /vo/fault_status
    /vo/healthy
